/*********************************************************************************** 
 * $URL: https://source.etudes.org/svn/apps/jforum/tags/2.27/jforum-tool/src/java/org/etudes/jforum/util/user/JForumUserUtil.java $ 
 * $Id: JForumUserUtil.java 9205 2014-11-14 19:01:50Z murthyt $ 
 *********************************************************************************** 
 * 
 ...
 * @version $Id: JForumUserUtil.java 9205 2014-11-14 19:01:50Z murthyt $
 */
public class JForumUserUtil {
	 ...
	 * 			false - if user is not observer
	 */
	public static boolean isEtudesObserver(String sakaiUserId)
	{
		...
		if (jforumSecurityService != null)
		{
			return jforumSecurityService.isEtudesObserver(ToolManager.getCurrentPlacement().getContext(), sakaiUserId);
		}
 ...
 * checks if user is active in site
 * @param userId
 * @checkOtherUsers will check other users in the same request 
 * @return - true if user is active
 * 			 false if usr is not active 
 */
public static boolean isUserActive(String sakaiUserId, boolean checkOtherUsers)
{
	if (sakaiUserId == null || sakaiUserId.trim().length() == 0)
	{
		return false;
	...
	if (checkOtherUsers)
	{
		return isUserActive(sakaiUserId);
	}
	
	JForumSecurityService jforumSecurityService = (JForumSecurityService)ComponentManager.get("org.etudes.api.app.jforum.JForumSecurityService");
	
	if (!jforumSecurityService.isUserActive(ToolManager.getCurrentPlacement().getContext(), sakaiUserId, false))
	{		
		return false;
	}
 ...
 * @param sakaiUserId	User Sakai id
 * 
 * @return	true - if user is facilitator
 * 			false - if user is not facilitator
 */
public static boolean isUserFacilitator(String sakaiUserId)
{
	if (sakaiUserId == null || sakaiUserId.trim().length() == 0)
	{
		return false;
	...
	if (jforumSecurityService != null)
	{
		return jforumSecurityService.isUserFacilitator(ToolManager.getCurrentPlacement().getContext(), sakaiUserId);
	}
