/**********************************************************************************
 * $URL: https://source.etudes.org/svn/apps/jforum/tags/2.27/jforum-tool/src/java/org/etudes/jforum/view/admin/SpecialAccessAction.java $ 
 * $Id: SpecialAccessAction.java 9155 2014-11-10 19:35:49Z murthyt $ 
 *********************************************************************************** 
 * 
...
public class SpecialAccessAction extends AdminCommand
{
	...
	public void editTopic() throws Exception
	{
		...
		JForumPostService jforumPostService = (JForumPostService) ComponentManager.get("org.etudes.api.app.jforum.JForumPostService");
		org.etudes.api.app.jforum.Topic topic = jforumPostService.getTopic(specialAccess.getTopicId());
		...
		JForumForumService jforumForumService = (JForumForumService) ComponentManager.get("org.etudes.api.app.jforum.JForumForumService");
		List<org.etudes.api.app.jforum.User> users = jforumForumService.getForumParticipantUsers(topic.getForumId());
		this.context.put("users", users);

		this.context.put("action", "editTopicSave");

		this.setTemplateName(TemplateKeys.SPECIAL_ACCESS_TOPIC_FORM);
	}
	
	/**
	 * Edit the existing special access of topic user
	 * 
	 * @throws Exception
	 */
	public void editTopicSave() throws Exception
	{
		boolean isfacilitator = JForumUserUtil.isJForumFacilitator(UserDirectoryService.getCurrentUser().getId()) || SecurityService.isSuperUser();
		...
		String topicSpecialAccess = this.request.getParameter("topic_special_access");
		...
		if (topicSpecialAccess != null)
		{
			editTopicSpecialAccess(topic);
		}
	...
	public void insertTopic() throws Exception
	{
		...
		this.context.put("action", "insertTopicSave");
		
		this.setTemplateName(TemplateKeys.SPECIAL_ACCESS_TOPIC_FORM);
	}
	
	/**
	 * Saved the topic special access
	 * 
	 * @throws Exception
	 */
	public void insertTopicSave() throws Exception
	{
		...
		String topicSpecialAccess = this.request.getParameter("topic_special_access");
		String topicGroupSpecialAccess = this.request.getParameter("topic_group_special_access");
		...
		if (topicSpecialAccess != null)
		{
			addTopicSpecialAccess(topic);
		}
		else if (topicGroupSpecialAccess != null)
		{
			addTopicSpecialAccess(topic);
		}	
	...
	/**
	 * Show topic special access list
	 * 
	 * @throws Exception
	 */
	public void showTopicList() throws Exception
	{
		...
		JForumSpecialAccessService jforumSpecialAccessService = (JForumSpecialAccessService) ComponentManager.get("org.etudes.api.app.jforum.JForumSpecialAccessService");
		List<org.etudes.api.app.jforum.SpecialAccess> topicSpecialAccessList = jforumSpecialAccessService.getByTopic(topic.getForumId(), topicId);
		
		// TODO: remove the special access and delete the user if user is inactive or removed from the site
					
		this.context.put("topicSpecialAccessList", topicSpecialAccessList);
		
		this.context.put("action", "delete");
		this.setTemplateName(TemplateKeys.SPECIAL_ACCESS_TOPIC_LIST);
	}
	...
	/**
	 * Add or modify topic special access
	 * 
	 * @param topic		Topic
	 * 
	 * @param specialAccess		Special access
	 ...
	 */
	protected void addEditSpecialAccess(org.etudes.api.app.jforum.Topic topic, org.etudes.api.app.jforum.SpecialAccess specialAccess) throws ParseException
	{
		// open date
		String startDateParam = this.request.getParameter("start_date");
		if (startDateParam != null && startDateParam.trim().length() > 0)
		...
		if (userIds != null){
			...
			for (int i = 0; i < userIds.length; i++) 
			{
				if (userIds[i] != null && userIds[i].trim().length() > 0) 
				{
					...
					{
						if (logger.isWarnEnabled()) logger.warn(this.getClass().getName()+".editTopicSpecialAccess() : Error while parsing the userId.", e);
					}
				}
			}
		}
	...
	/**
	 * Add or modify user topic special access
	 * 
	 * @param topic			Topic
	 * 
	 * @param specialAccess	Special access
	 ...
	 * @throws Exception
	 */
	protected void addTopicSpecialAccess(org.etudes.api.app.jforum.Topic topic) throws Exception
	{
		JForumSpecialAccessService jforumSpecialAccessService = (JForumSpecialAccessService) ComponentManager.get("org.etudes.api.app.jforum.JForumSpecialAccessService");
		org.etudes.api.app.jforum.SpecialAccess specialAccess = jforumSpecialAccessService.newSpecialAccess(topic.getForumId(), topic.getId());
		...
		try
		{
			addEditSpecialAccess(topic, specialAccess);
		}
		catch (ParseException e)
		{
			this.context.put("errorMessage", I18n.getMessage("Forums.Forum.DateParseError"));
	...
	/**
	 * Modify topic special access
	 * 
	 * @param topic		Topic
	 * 
	 * @throws Exception
	 */
	protected void editTopicSpecialAccess(org.etudes.api.app.jforum.Topic topic) throws Exception
	{
		int specialAccessId = this.request.getIntParameter("special_access_id");
		...
		jforumSpecialAccessService.modifyTopicSpecialAccess(specialAccess);
	...
	protected void getForumUsers(org.etudes.api.app.jforum.Forum forum) throws IdUnusedException
	{
		...
		if ((forum.getGroups() != null) && forum.getGroups().size() > 0)
		{
			...
			Map<String, org.etudes.api.app.jforum.User> usersMap = new HashMap<String, org.etudes.api.app.jforum.User>();
			try 
			{
				JForumUserService jforumUserService = (JForumUserService) ComponentManager.get("org.etudes.api.app.jforum.JForumUserService");
				...
				{
					org.etudes.api.app.jforum.User user = (org.etudes.api.app.jforum.User) userIterator.next();
					if (jforumSecurityService.isJForumFacilitator(ToolManager.getCurrentPlacement().getContext(), user.getSakaiUserId()))
					{
						userIterator.remove();
					}
			...
			List<org.etudes.api.app.jforum.User> forumGroupMembers = new ArrayList<org.etudes.api.app.jforum.User>();
			...
			Collections.sort(users, new Comparator<org.etudes.api.app.jforum.User>()
			{
				public int compare(org.etudes.api.app.jforum.User u1, org.etudes.api.app.jforum.User u2)
				{
