...
<#if attachmentsEnabled>
	...
	template += "&nbsp;&nbsp;<span class='gensmall'><a class='toolUiLinkU' href='javascript:removeAttach(#counter#)' class='gensmall'>${I18n.getMessage("Attachments.remove")}</a></span></td></tr>";
...
<#if attachments?exists>
	...
	function editAttachments()
	{
		...
		<#list attachments as a>
			...
			<#--if pmDraftEdit?? && pmDraftEdit && pmDraft??>
				attach_${a.id}["filename"] = "<a href='${JForumContext.encodeURL("/posts/downloadAttach/${a.id}")}'>${a.info.realFilename}</a>";			
			<#else>
				attach_${a.id}["filename"] = "${a.info.realFilename}";
			</#if-->
			
			attach_${a.id}["filename"] = "<a href='${JForumContext.encodeURL("/posts/downloadAttach/${a.id}")}' class='gen'>${a.info.realFilename}</a>";
