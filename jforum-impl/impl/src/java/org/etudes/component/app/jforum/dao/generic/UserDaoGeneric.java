/**********************************************************************************
 * $URL: https://source.etudes.org/svn/apps/jforum/tags/2.27/jforum-impl/impl/src/java/org/etudes/component/app/jforum/dao/generic/UserDaoGeneric.java $ 
 * $Id: UserDaoGeneric.java 7206 2014-01-22 01:32:00Z murthyt $ 
 *********************************************************************************** 
 * 
...
public abstract class UserDaoGeneric implements UserDao
{
	 ...
	 * {@inheritDoc}
	 */
	public int selectUserCategoryPostsCount(int userId, int categoryId)
	{
		...
		sql.append("SELECT COUNT(p.post_id) AS num_messages ");
		sql.append("FROM jforum_posts p, jforum_forums f ");
		sql.append("WHERE p.forum_id = f.forum_id ");
		...
		this.sqlService.dbRead(sql.toString(), fields, new SqlReader()
		{
			public Object readSqlResultRecord(ResultSet result)
			{
				...
				catch (SQLException e)
				{
					if (logger.isWarnEnabled())
					{
						logger.warn("selectUserCategoryPostsCount: " + e, e);
					}
					return null;
				}
			}
	 ...
	 * {@inheritDoc}
	 */
	public int selectUserForumPostsCount(int userId, int forumId)
	{
		...
		this.sqlService.dbRead(sql.toString(), fields, new SqlReader()
		{
			public Object readSqlResultRecord(ResultSet result)
			{
				...
				catch (SQLException e)
				{
					if (logger.isWarnEnabled())
					{
						logger.warn("selectUserForumPostsCount: " + e, e);
					}
					return null;
				}
			}
	 ...
	 * {@inheritDoc}
	 */
	public int selectUserTopicPostsCount(int userId, int topicId)
	{
		...
		sql.append("SELECT COUNT(p.post_id) AS num_messages ");
		sql.append("FROM jforum_posts p ");
		...
		this.sqlService.dbRead(sql.toString(), fields, new SqlReader()
		{
			public Object readSqlResultRecord(ResultSet result)
			{
				...
				catch (SQLException e)
				{
					if (logger.isWarnEnabled())
					{
						logger.warn("selectUserTopicPostsCount: " + e, e);
					}
					return null;
				}
			}
