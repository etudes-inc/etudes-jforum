/**********************************************************************************
 * $URL: https://source.etudes.org/svn/apps/jforum/tags/2.27/jforum-impl/impl/src/java/org/etudes/component/app/jforum/JForumCategoryServiceImpl.java $ 
 * $Id: JForumCategoryServiceImpl.java 10892 2015-05-19 23:03:57Z murthyt $ 
 *********************************************************************************** 
 * 
...
public class JForumCategoryServiceImpl implements JForumCategoryService
{
	...
	public void assignZeroCategoryNonSubmitters(int categoryId, String evaluatedBySakaiUserId) throws JForumAccessException
	{
		...
		if (category == null || !category.isGradable() || evaluatedBySakaiUserId == null || evaluatedBySakaiUserId.trim().length() == 0)
		{
			return;
		}
		...
		jforumGradeService.assignZeroCategoryNonSubmitters(categoryId, evaluatedBySakaiUserId);
	}
	...
	public int createCategory(Category category) throws JForumAccessException
	{
		...
		if ((category.getGrade() != null && category.getGrade().isAddToGradeBook()) && (site != null)
		...
		{
			if (!etudesGradebookService.isTitleAvailable(category.getContext(), category.getCreatedBySakaiUserId(), category.getTitle()))
			{
				category.getGrade().setAddToGradeBook(Boolean.FALSE);
			}
		}
		...
		// add to UCB gradebook
		if (grade != null && grade.isAddToGradeBook() && (site != null) && (site.getToolForCommonId(JForumGradeService.GRADEBOOK_TOOL_ID) != null))
		{
	...
	public Category getCategoryEvaluations(String context, int categoryId)
	{
		...
		if (category != null)
		{
			...
			try
			{
				evaluations = this.jforumGradeService.getCategoryEvaluationsWithPosts(categoryId, Evaluation.EvaluationsSort.last_name_a, this.userDirectoryService.getCurrentUser().getId(), true);
			...
			catch (JForumAccessException e)
			{
				...
				{
					logger.warn("getCategoryEvaluations(String context, int categoryId) - context:"+ context +": Jforum Access Exception");
				}
			}
	 ...
	 * {@inheritDoc}
	 */
	public Category getCategoryEvaluations(String context, int categoryId, boolean includeBlockedDropped)
	{
		if (context == null) throw new IllegalArgumentException();
		...
		if (category != null)
		{
			...
			try
			{
				evaluations = this.jforumGradeService.getCategoryEvaluationsWithPosts(categoryId, Evaluation.EvaluationsSort.last_name_a, this.userDirectoryService.getCurrentUser().getId(), true, true);
				category.getEvaluations().addAll(evaluations);
			}
			catch (JForumAccessException e)
			{
				if (logger.isWarnEnabled()) 
				{
					logger.warn("getCategoryEvaluations(String context, int categoryId, boolean includeBlockedDropped) - context:"+ context +": Jforum Access Exception");
				}
			}
	...
	public Category getForumEvaluations(String context, int forumId)
	{
		...
		if (forum != null)
		{
			...
			if (category != null)
			{
				...
				try
				{
					evaluations = jforumGradeService.getForumEvaluationsWithPosts(forumId, Evaluation.EvaluationsSort.last_name_a, this.userDirectoryService.getCurrentUser().getId(), true);
				...
				catch (JForumAccessException e)
				{
					if (logger.isWarnEnabled()) 
					{
						logger.warn("getForumEvaluations(String context, int forumId) - context:"+ context +": Jforum Access Exception");
					}
				}
	 ...
	 * {@inheritDoc}
	 */
	public Category getForumEvaluations(String context, int forumId, boolean includeBlockedDropped)
	{
		if (context == null) throw new IllegalArgumentException();
		Category category = null;
		...
		if (forum != null)
		{
			...
			if (category != null)
			{
				...
				try
				{
					evaluations = jforumGradeService.getForumEvaluationsWithPosts(forumId, Evaluation.EvaluationsSort.last_name_a, this.userDirectoryService.getCurrentUser().getId(), true, includeBlockedDropped);
					forum.getEvaluations().addAll(evaluations);
				}
				catch (JForumAccessException e)
				{
					if (logger.isWarnEnabled()) 
					{
						logger.warn("getForumEvaluations(String context, int forumId, boolean includeBlockedDropped) - context:"+ context +": Jforum Access Exception");
					}
				}
		...
		if (topic != null)
		{
			...
			if (forum != null)
			{
				...
				if (category != null)
				{
					...
					try
					{
						evaluations = jforumGradeService.getTopicEvaluationsWithPosts(topicId, Evaluation.EvaluationsSort.last_name_a, this.userDirectoryService.getCurrentUser().getId(), true);
					...
					catch (JForumAccessException e)
					{
						if (logger.isWarnEnabled()) 
						{
							logger.warn("getTopicEvaluations(String context, int topicId) - context:"+ context +": Jforum Access Exception");
						}
					}
	 ...
	 * {@inheritDoc}
	 */
	public Category getTopicEvaluations(String context, int topicId, boolean includeBlockedDropped)
	{
		if (context == null) throw new IllegalArgumentException();
		Category category = null;
		...
		if (topic != null)
		{
			...
			if (forum != null)
			{
				...
				if (category != null)
				{
					...
					try
					{
						evaluations = jforumGradeService.getTopicEvaluationsWithPosts(topicId, Evaluation.EvaluationsSort.last_name_a, this.userDirectoryService.getCurrentUser().getId(), true, true);
						topic.getEvaluations().addAll(evaluations);
					}
					catch (JForumAccessException e)
					{
						if (logger.isWarnEnabled()) 
						{
							logger.warn("getTopicEvaluations(String context, int topicId, boolean includeBlockedDropped) - context:"+ context +": Jforum Access Exception");
						}
					}
...
//		sql.append("AND (t.topic_grade = "+ Topic.GRADE_YES +" OR t.topic_export = "+ Topic.EXPORT_YES +") ");
	...
	public List<Category> getUserContextCategories(String context, String userId)
	{
		...
		boolean facilitator = jforumSecurityService.isJForumFacilitator(context, userId);
		
		boolean participant = false;
		if (!facilitator)
		{
			participant = jforumSecurityService.isJForumParticipant(context, userId);
		}
		...
		boolean observer = jforumSecurityService.isEtudesObserver(context, userId);
		...
		for (Iterator<Category> catIter = categories.iterator(); catIter.hasNext();) 
		{
		   ...
		   ((CategoryImpl)category).setCurrentSakaiUserId(userId);
		    ...
		    if (participant && !observer)
			{
				...
				for (Iterator<Forum> forumIter = category.getForums().iterator(); forumIter.hasNext(); ) 
				{
					...
					if (f.getAccessType() == Forum.ForumAccess.GROUPS.getAccessType())
					{
						...
						if (!userInGroup)
						{
							if (f.getGroupsReadonyAccessType() == Forum.ForumGroupsReadonlyAccessType.IMMEDIATE.getForumGroupsReadonyAccessType())
							{
								((ForumImpl)f).setGroupUser(false);
								((ForumImpl)f).setGroupReadonlyUser(true);
							}
							else if (f.getGroupsReadonyAccessType() == Forum.ForumGroupsReadonlyAccessType.DUEDATE.getForumGroupsReadonyAccessType())
							{
								if ((f.getAccessDates() != null) && (f.getAccessDates().getDueDate() != null) && (f.getAccessDates().getDueDate().before(currentTime)))
								{
									((ForumImpl)f).setGroupUser(false);
									((ForumImpl)f).setGroupReadonlyUser(true);
								}
								else if ((f.getAccessDates() == null) || (f.getAccessDates().getDueDate() == null))
								{
									((ForumImpl)f).setGroupUser(false);
									((ForumImpl)f).setGroupReadonlyUser(true);
								}
				...
				if (category.isGradable())
				{
					...
					if (userSiteEvaluationsMap == null)
					{
						...
						userSiteEvaluations = jforumGradeService.getUserSiteEvaluations(context, user.getId());
					...
					if (userSiteEvaluationsMap != null)
					{
						Evaluation userEvaluation = userSiteEvaluationsMap.get(new Integer(category.getGrade().getId()));
						...
						((CategoryImpl)category).setUserEvaluation(userEvaluation);
					}
				}
				...
				for(Forum f : catForumList)
				{
					...
					if (f.getGradeType() == Grade.GradeType.FORUM.getType())
					{
						...
						{							
							...
							if (userSiteEvaluationsMap != null)
							{
								Evaluation userEvaluation = userSiteEvaluationsMap.get(new Integer(f.getGrade().getId()));
								
								// get forum user posts count
								int userForumPostsCount = jforumUserService.getUserForumPostsCount(user.getId(), f.getId());
								...
								((ForumImpl)f).setUserEvaluation(userEvaluation);
							}
						}
					}
				}
		    ...
		    else
		    {
				...
				{
					((ForumImpl)forum).setCurrentSakaiUserId(userId);
	 ...
	 * {@inheritDoc}
	 */
	public int getUserUnreadTopicsCount(String context, String sakaiUserId)
	{
		if (context == null || context.trim().length() == 0)
		{
		...
		for (Iterator<Category> catIter = categories.iterator(); catIter.hasNext();) 
		{
		    ...
		    if (!(jforumSecurityService.isJForumFacilitator(context, sakaiUserId)))
			{
				...
				{
					// count unread topics
					unreadTopicsCount += countUserForumUnreadTopics(context, forum, user); 
				}
			}
		    else
		    {
	...
	public void modifyCategory(Category category) throws JForumAccessException, JForumGradesModificationException
	{
		...
		// check duplicate titles in etudes gradebook
		if ((category.isGradable()) && (category.getGrade() != null && category.getGrade().isAddToGradeBook()) && (site != null)
								&& ((site.getToolForCommonId(JForumGradeService.GRADEBOOK_TOOL_ID) == null) && (etudesGradebookService.isToolAvailable(category.getContext()))))
		{
			// check for current item
			if (!etudesGradebookService.isTitleDefined(category.getContext(), category.getModifiedBySakaiUserId(), category.getTitle(), String.valueOf(category.getId()), GradebookItemType.category))
			{
				if (!etudesGradebookService.isTitleAvailable(category.getContext(), category.getModifiedBySakaiUserId(), category.getTitle()))
				{
					category.getGrade().setAddToGradeBook(Boolean.FALSE);
				}
			}
	...
	/**
	 * Get the user unread topics count of the forum 
	 * 
	 * @param context	The contest or course id
	 * 
	 * @param forum		Forum
	 * 
	 * @param user		User
	 * 
	 * @return	Unread topics count of forum
	 */
	protected int countUserForumUnreadTopics(String context, Forum forum, User user)
	{
		if (user == null || forum == null || context == null || context.trim().length() == 0)
		{
			return 0;
		}
		...
		boolean facilitator = jforumSecurityService.isJForumFacilitator(context, user.getSakaiUserId());
		
		/*filter topics for participants by dates, special access etc*/
		...
		{
			/* no special access topics and special access topics*/			
			if (forumTopicsSpecialAccess.size() > 0)
			{
		...
		for (Topic t : topicsLastPosttimes)
		{
			if (!facilitator)
			{
				...
				{
					if ((t.getAccessDates().getOpenDate() != null) || (t.getAccessDates().getDueDate() != null) || (t.getAccessDates().getAllowUntilDate() != null))
					{
						if (userSpecialAccessNoAccessMap.containsKey(t.getId()))
						{
		...
		// get the unread topics count of the user accessible topics
		for (Topic lastPosttimeTopic : lastPosttimeTopicsList)
		{
			Topic lastVistiedTopic = visitedTopicsMap.remove(Integer.valueOf(lastPosttimeTopic.getId()));
			
			if (lastVistiedTopic != null)
			{
				// if topic is marked as unread count the topic as unread
				if (!lastVistiedTopic.getRead())
				{
					unreadTopicsCount++;
				}
				else if (lastVistiedTopic.getTime().before(lastPosttimeTopic.getTime()))
				{
					unreadTopicsCount++;
				}
			}
		...
		// count the topics if the last post of the post is by the user and marked as unread
		if (visitedTopicsMap.size() > 0)
		{
			for (Topic visitedTopic : visitedTopicsMap.values())
			{
					if (!visitedTopic.getRead())
					{
						unreadTopicsCount++;
	...
	{
		CategoryImpl category = new CategoryImpl(this.jforumGradeService);
	...
	{
		TopicImpl topic = new TopicImpl(this.jforumForumService, this.jforumGradeService);
