/**********************************************************************************
 * $URL: https://source.etudes.org/svn/apps/jforum/tags/2.27/jforum-impl/impl/src/java/org/etudes/component/app/jforum/dao/generic/GradeDaoGeneric.java $ 
 * $Id: GradeDaoGeneric.java 7928 2014-05-07 19:10:15Z murthyt $ 
 *********************************************************************************** 
 * 
...
public abstract class GradeDaoGeneric implements GradeDao
{
		...
		this.sqlService.dbRead(sql.toString(), fields, new SqlReader()
		{
			public Object readSqlResultRecord(ResultSet result)
			{
				...
				catch (SQLException e)
				{
					...
					{
						logger.warn("checkScheduledGradeToGradebook: " + e, e);
					}
					return null;
				}
			}
			...
			this.sqlService.dbRead(sql.toString(), fields, new SqlReader()
			{
				public Object readSqlResultRecord(ResultSet result)
				{
					...
					catch (SQLException e)
					{
						if (logger.isWarnEnabled())
						{
							logger.warn("selectCategoryGradableForumsCount: " + e, e);
						}
						return null;
					}
				}
		...
		this.sqlService.dbRead(sql.toString(), fields, new SqlReader()
		{
			public Object readSqlResultRecord(ResultSet result)
			{
				try
				{
				    ...
				    {
				    	((EvaluationImpl)evaluation).setReviewedDate(null);
				    }
				...
				catch (SQLException e)
				{
					if (logger.isWarnEnabled())
					{
						logger.warn("selectEvaluations: " + e, e);
					}
					return null;
				}
			}
			...
			this.sqlService.dbRead(sql.toString(), fields, new SqlReader()
			{
				public Object readSqlResultRecord(ResultSet result)
				{
					...
					catch (SQLException e)
					{
						if (logger.isWarnEnabled())
						{
							logger.warn("selectForumTopicEvaluationsCount: " + e, e);
						}
						return null;
					}
				}
	...
	public Evaluation selectUserCategoryEvaluation(int categoryId, int userId, boolean checkLatePosts)
	{
		...
		StringBuilder sql = new StringBuilder();
		sql.append("SELECT p.user_id, COUNT(p.post_id) AS user_posts_count, MIN(post_time) AS first_post_time, MAX(post_time) AS last_post_time, u.user_fname AS user_fname,u.user_lname AS user_lname, u.sakai_user_id, u.username "); 
		...
		final List<Date> userFirstPostTime = new ArrayList<Date>();
			...
			public Object readSqlResultRecord(ResultSet result)
			{
				try
				{
					...
					if (userPostCount > 0)
					{
						...
						userFirstPostTime.add(result.getTimestamp("first_post_time"));
					}
	...
	public Evaluation selectUserForumEvaluation(int forumId, int userId, boolean checkLatePosts)
	{
		...
		// user posts count and user last post time
		StringBuilder sql = new StringBuilder();
		sql.append("SELECT s.user_id, COUNT(p.post_id) AS user_posts_count, MIN(post_time) AS first_post_time, MAX(post_time) AS last_post_time, u.user_fname AS user_fname,u.user_lname AS user_lname, u.sakai_user_id, u.username ");		
	 ...
	 * {@inheritDoc}
	 */
	public List<Evaluation> selectUserForumTopicAccessibleEvaluations(int forumId, int userId)
	{
		if (forumId <= 0 || userId <= 0)
		{
			throw new IllegalArgumentException("forum id or user id not valid");
		}
		...
		sql = new StringBuilder();
		sql.append("SELECT e.evaluation_id, e.grade_id, e.user_id, e.sakai_user_id, e.score, e.comments, e.evaluated_by, e.evaluated_date, e.released, e.reviewed_date ");
		sql.append("FROM jforum_evaluations e, jforum_grade g WHERE e.user_id = ? AND e.released = 1 ");
		sql.append("AND e.grade_id = g.grade_id AND g.forum_id = ? AND g.topic_id > 0");
		...
		this.sqlService.dbRead(sql.toString(), fields, new SqlReader()
		{
			public Object readSqlResultRecord(ResultSet result)
			{
				try
				{
					Evaluation evaluation = fillEvaluation(result);
				...
				catch (SQLException e)
				{
					if (logger.isWarnEnabled())
					{
						logger.warn("selectUserForumTopicEvaluations: " + e, e);
					}
					return null;
				}
			}
	 ...
	 * {@inheritDoc}
	 */
	public int selectUserForumTopicAccessibleEvaluationsCount(int forumId, int userId)
	{
		if (forumId <= 0 || userId <= 0)
		{
			throw new IllegalArgumentException("forum id or user id not valid");
		...
		sql = new StringBuilder();
		sql.append("SELECT COUNT(e.evaluation_id) As eval_count ");
		sql.append("FROM jforum_evaluations e, jforum_grade g WHERE e.user_id = ? AND e.released = 1 ");
		sql.append("AND e.grade_id = g.grade_id AND g.forum_id = ? AND g.topic_id > 0");
			...
			this.sqlService.dbRead(sql.toString(), fields, new SqlReader()
			{
				public Object readSqlResultRecord(ResultSet result)
				{
					...
					catch (SQLException e)
					{
						if (logger.isWarnEnabled())
						{
							logger.warn("selectUserForumTopicEvaluationsCount: " + e, e);
						}
						return null;
					}
				}
	 ...
	 * {@inheritDoc}
	 */
	public List<Evaluation> selectUserReleasedEvaluations(String siteId, int userId)
	{
		if (siteId == null || siteId.trim().length() == 0 || userId <= 0)
		{
			throw new IllegalArgumentException("Site id or user id is not valid");
		}
		...
		StringBuilder sql = new StringBuilder();
		sql.append("SELECT e.evaluation_id, e.grade_id, e.user_id, e.sakai_user_id, e.score, e.comments, e.evaluated_by, e.evaluated_date, e.released, e.reviewed_date "); 
		sql.append("FROM jforum_grade g, jforum_evaluations e ");
		sql.append("WHERE g.grade_id = e.grade_id ");
		sql.append("AND g.context = ? ");
		sql.append("AND e.user_id = ? ");
		sql.append("AND e.released = 1");
		...
		this.sqlService.dbRead(sql.toString(), fields, new SqlReader()
		{
			public Object readSqlResultRecord(ResultSet result)
			{
				...
				catch (SQLException e)
				{
					if (logger.isWarnEnabled())
					{
						logger.warn("selectUserReleasedEvaluations : " + e, e);
					}
					return null;
				}
			}
	 ...
	 * {@inheritDoc}
	 */
	public List<Evaluation> selectUserSiteEvaluations(String context, int userId)
	{
		...
		this.sqlService.dbRead(sql.toString(), fields, new SqlReader()
		{
			public Object readSqlResultRecord(ResultSet result)
			{
				...
				catch (SQLException e)
				{
					if (logger.isWarnEnabled())
					{
						logger.warn("selectUserSiteEvaluations: " + e, e);
					}
					return null;
				}
			}
					...
					else 
					{
						if (evaluation.getId() == exisUserEvaluation.getId())
						{
							...
							else
							{
								...
								/* update only if existing evaluation is changed*/
								if (checkEvaluationChanges(exisUserEvaluation, evaluation))
								{
									// update evaluation
									((EvaluationImpl) evaluation).setEvaluatedDate(new Date());
	...
	protected String applySort(Evaluation.EvaluationsSort evalSort)
	{
		...
		if (evalSort == null)
		{
			sql = " ORDER BY user_lname ASC, user_fname ASC";
			return sql;
		}
		...
		switch (evalSort)
		{
			...
			case last_name_d:
			{
				sql = " ORDER BY user_lname DESC, user_fname DESC";
				break;
			}
			case total_posts_a:
			{
				//added user_lname in order to return the rows in the same order with posts count or with out posts count
				sql = " ORDER BY user_posts_count ASC, user_lname ASC, user_fname ASC";
				break;
			}
			case total_posts_d:
			{
				//added user_lname in order to return the rows in the same order with posts count or with out posts count
				sql = " ORDER BY user_posts_count DESC, user_lname ASC, user_fname ASC";
				break;
			}
			case last_post_date_a:
			{
				//added user_lname in order to return the rows in the same order with posts count or with out posts count
				sql = " ORDER BY last_post_time ASC, user_lname ASC, user_fname ASC";
				break;
			}
			case last_post_date_d:
			{
				//added user_lname in order to return the rows in the same order with posts count or with out posts count
				sql = " ORDER BY last_post_time DESC, user_lname ASC, user_fname ASC";
				
				break;
			}
			case first_post_date_a:
			{
				//added user_lname in order to return the rows in the same order with posts count or with out posts count
				sql = " ORDER BY first_post_time ASC, user_lname ASC, user_fname ASC";
				break;
			}
			case first_post_date_d:
			{
				//added user_lname in order to return the rows in the same order with posts count or with out posts count
				sql = " ORDER BY first_post_time DESC, user_lname ASC, user_fname ASC";
	...
	/**
	 * Sort string of the sql with user late posts
	 * 
	 * @param evalSort	Evaluation sort
	 * 
	 * @return	The sort string
	 */
	protected String applySortLatePosts(Evaluation.EvaluationsSort evalSort)
	{
		String sql = null;
		if (evalSort == null)
		{
		...
		switch (evalSort)
		{
			...
			case total_posts_a:
			{
				sql = " ORDER BY user_late_posts_count ASC, user_lname ASC, user_fname ASC";
				break;
			}
			case total_posts_d:
			{
				sql = " ORDER BY user_late_posts_count DESC, user_lname ASC, user_fname ASC";
				break;
			}
			default:
			{
	...
	protected boolean checkEvaluationChanges(Evaluation exisEval, Evaluation modEval)
	{
		...
		else
		{
			...
			else if (((exisComments == null || exisComments.trim().length() == 0) && (modComments != null && modComments.trim().length() > 0)) || ((exisComments != null && exisComments.trim().length() > 0) && (modComments == null || modComments.trim().length() == 0)))
			{
				return true;
			}
			else if (exisComments != null && modComments!= null && !exisComments.equalsIgnoreCase(modComments))
			{
				return true;
			}
	 ...
	 * @throws SQLException
	 */
	protected Evaluation fillEvaluation(ResultSet result) throws SQLException
	{
		Evaluation evaluation = new EvaluationImpl();
		((EvaluationImpl) evaluation).setId(result.getInt("evaluation_id"));
		((EvaluationImpl) evaluation).setGradeId(result.getInt("grade_id"));
		...
		if (result.getDate("evaluated_date") != null)
		{
			Timestamp evaluatedDate = result.getTimestamp("evaluated_date");
			((EvaluationImpl) evaluation).setEvaluatedDate(evaluatedDate);
		}
		else
		{
			((EvaluationImpl) evaluation).setEvaluatedDate(null);
	...
	protected Evaluation fillEvaluationUserInfo(ResultSet rs) throws SQLException
	{
		...
		((EvaluationImpl)evaluation).setFirstPostTime(rs.getTimestamp("first_post_time"));
	...
	protected List<Evaluation> selectCategoryEvaluations(int categoryId, boolean all, Evaluation.EvaluationsSort evalSort)
	{
		...
		if (dueDate != null)
		{
			...
			{	
				...
				sql.append("AND p.forum_id IN (SELECT forum_id FROM jforum_forums f where f.categories_id = ?) ");
				sql.append("GROUP BY s.user_id, u.user_fname, u.user_lname, u.sakai_user_id, u.username ");
				sql.append(applySortLatePosts(evalSort));
