/**********************************************************************************
 * $URL: https://source.etudes.org/svn/apps/jforum/tags/2.27/jforum-impl/impl/src/java/org/etudes/component/app/jforum/dao/mysql/TopicDaoMysql.java $ 
 * $Id: TopicDaoMysql.java 8540 2014-08-29 22:18:25Z murthyt $ 
 *********************************************************************************** 
 * 
...
public class TopicDaoMysql extends TopicDaoGeneric implements TopicDao
{
	...
	protected List<Topic> getForumTopics(int forumId, int startFrom, int count)
	{
		...
		int forumTopicOrder = 0;
		
		if (forum != null)
		{
			forumTopicOrder = forum.getTopicOrder();
		}
		...
		if ((forumTopicOrder > 0) && ((forumTopicOrder == Forum.ForumTopicOrder.OPEN_DATE.getTopicOrder()) || (forumTopicOrder == Forum.ForumTopicOrder.DUE_DATE.getTopicOrder())))
		{
			if (startFrom == 0 && count == 0)
			{	
				topics = new ArrayList<Topic>();
				
				if (forumTopicOrder == Forum.ForumTopicOrder.OPEN_DATE.getTopicOrder())
				{
					...
					sql.append("ORDER BY t.start_date DESC, t.topic_type DESC, t.topic_export DESC, p2.post_time DESC, t.topic_last_post_id DESC ");*/
					...
					sql.append("AND p2.post_id = t.topic_last_post_id ");
					sql.append("AND u2.user_id = p2.user_id ");
					sql.append("ORDER BY t.topic_type DESC, CASE when t.start_date IS NULL THEN 1 ELSE 0 END, t.start_date, t.topic_export DESC, p2.post_time DESC, t.topic_last_post_id DESC ");
				
					int i = 0; 
					fields = new Object[1];
					fields[i++] = forumId;
				}
				else if (forumTopicOrder == Forum.ForumTopicOrder.DUE_DATE.getTopicOrder())
				{
					sql = new StringBuilder();
					...
					sql.append("AND p2.post_id = t.topic_last_post_id ");
					sql.append("AND u2.user_id = p2.user_id ");
					sql.append("ORDER BY t.end_date DESC, t.topic_type DESC, t.topic_export DESC, p2.post_time DESC, t.topic_last_post_id DESC ");*/
					...
					sql.append("AND p2.post_id = t.topic_last_post_id ");
					sql.append("AND u2.user_id = p2.user_id ");
					sql.append("ORDER BY t.topic_type DESC, CASE when t.end_date IS NULL THEN 1 ELSE 0 END, t.end_date, t.topic_export DESC, p2.post_time DESC, t.topic_last_post_id DESC ");
				...
				topics.addAll(readTopics(sql.toString(), fields));
			}
			else
			{
				topics = new ArrayList<Topic>();
		...
		else
		{
			...
			if (startFrom == 0 && count == 0)
			{
				...
				sql.append("AND p2.post_id = t.topic_last_post_id ");
				sql.append("AND u2.user_id = p2.user_id ");
				sql.append("ORDER BY t.topic_type DESC, t.topic_export DESC, p2.post_time DESC, t.topic_last_post_id DESC ");
	 ...
	 * {@inheritDoc}
	 */
	protected List<Topic> getForumTopics(int forumId, Topic.TopicSort topicSort, int startFrom, int count)
	{
		if (startFrom < 0 || count < 0)
		{
			return new ArrayList<Topic>();
		...
		if (startFrom == 0 && count == 0)
		{
			...
			sql.append("AND p2.post_id = t.topic_last_post_id ");
			sql.append("AND u2.user_id = p2.user_id ");
			if (topicSort == Topic.TopicSort.name_d)
			{
				sql.append("ORDER BY posted_by_lname DESC, posted_by_fname DESC ");
			}
			else if (topicSort == Topic.TopicSort.name_a)
			{
				sql.append("ORDER BY posted_by_lname ASC, posted_by_fname ASC ");
			}
			else
			{
				sql.append("ORDER BY t.topic_type DESC, t.topic_export DESC, p2.post_time DESC, t.topic_last_post_id DESC ");
	...
	protected Post getPostById(int postId)
	{
		...
		sql.append("p.enable_html, p.enable_smilies, p.enable_sig, p.post_edit_time, p.post_edit_count, p.status, p.user_likes_count, pt.post_subject, pt.post_text, ");
	...
	protected List<Post> getPostsByCategoryByUser(int categoryId, int userId)
	{
		...
		sql.append("enable_html, enable_smilies, enable_sig, post_edit_time, post_edit_count, status, p.user_likes_count, pt.post_subject, pt.post_text, p.need_moderate, ");
		//sql.append("u.username, u.sakai_user_id, u.user_fname, u.user_lname, u.user_avatar, u.user_attachsig, u.user_sig ");
		sql.append("u.user_id, u.username, u.user_regdate, u.user_new_privmsg, u.user_unread_privmsg, u.user_last_privmsg, u.user_viewemail, u.user_attachsig, ");
		sql.append("u.user_allowhtml, u.user_allow_pm, u.user_notify, u.user_notify_pm, u.user_avatar, u.user_avatar_type, u.user_email, u.user_icq, ");
		sql.append("u.user_website, u.user_from, u.user_sig, u.user_sig_bbcode_uid, u.user_aim, u.user_yim, u.user_msnm, u.user_occ, u.user_interests, u.gender, u.user_fname, ");
	...
	protected List<Post> getPostsByForumByUser(int forumId, int userId)
	{
		...
		sql.append("enable_html, enable_smilies, enable_sig, post_edit_time, post_edit_count, status, p.user_likes_count, pt.post_subject, pt.post_text, username, p.need_moderate, ");
		//sql.append("u.username, u.sakai_user_id, u.user_fname, u.user_lname, u.user_avatar, u.user_attachsig, u.user_sig ");		
		sql.append("u.user_id, u.username, u.user_regdate, u.user_new_privmsg, u.user_unread_privmsg, u.user_last_privmsg, u.user_viewemail, u.user_attachsig, ");
		sql.append("u.user_allowhtml, u.user_allow_pm, u.user_notify, u.user_notify_pm, u.user_avatar, u.user_avatar_type, u.user_email, u.user_icq, ");
		sql.append("u.user_website, u.user_from, u.user_sig, u.user_sig_bbcode_uid, u.user_aim, u.user_yim, u.user_msnm, u.user_occ, u.user_interests, u.gender, u.user_fname, ");
	 ...
	 * {@inheritDoc}
	 */
	protected List<Post> getPostsByUser(String siteId, int userId)
	{
		if ((siteId == null || siteId.trim().length() == 0) || (userId <= 0))
		{
			throw new IllegalArgumentException("site or user information is missing.");
		}
		...
		sql.append("u.user_website, u.user_from, u.user_sig, u.user_sig_bbcode_uid, u.user_aim, u.user_yim, u.user_msnm, u.user_occ, u.user_interests, u.gender, u.user_fname, "); 
		sql.append("u.user_lname, u.sakai_user_id, u.user_facebook_account, u.user_twitter_account, u.user_lang ");
		sql.append("FROM jforum_sakai_course_categories cc, jforum_posts p, jforum_posts_text pt, jforum_users u, jforum_forums f ");
		sql.append("WHERE p.post_id = pt.post_id ");
		sql.append("AND p.forum_id = f.forum_id ");
		...
		this.sqlService.dbRead(sql.toString(), fields, new SqlReader()
		{
			public Object readSqlResultRecord(ResultSet result)
			{
				...
				catch (SQLException e)
				{
					if (logger.isWarnEnabled())
					{
						logger.warn("getPostsByUser: " + e, e);
					}
					return null;
				}
			}
