/**********************************************************************************
 * $URL: https://source.etudes.org/svn/apps/jforum/tags/2.27/jforum-impl/impl/src/java/org/etudes/component/app/jforum/dao/generic/TopicDaoGeneric.java $ 
 * $Id: TopicDaoGeneric.java 12733 2016-07-21 21:53:34Z murthyt $ 
 *********************************************************************************** 
 * 
...
public abstract class TopicDaoGeneric implements TopicDao
{
	...
	public boolean addUserPostLike(int postId, int userId)
	{
		...
		return postDao.addUserPostLike(postId, userId);
	}
	...
	{
		String sql = "SELECT user_id FROM jforum_topics_watch WHERE topic_id = ? AND user_id = ? AND forum_id = 0";
	 ...
	 * {@inheritDoc}
	 */
	public boolean isUserSubscribed(int topicId, int userId, int forumId)
	{
		if (topicId <= 0 || userId <= 0 || forumId <= 0)
		{
			throw new IllegalArgumentException("topic id or user id or forum id is missing");
		}
		
		String sql = "SELECT user_id FROM jforum_topics_watch WHERE topic_id = ? AND user_id = ? AND forum_id = ?";
		...
		this.sqlService.dbRead(sql.toString(), fields, new SqlReader()
		{
			public Object readSqlResultRecord(ResultSet result)
			{
				...
				catch (SQLException e)
				{
					if (logger.isWarnEnabled())
					{
						logger.warn("isUserSubscribed(topicId, userId, forumId): " + e, e);
					}
					return null;
				}
			}
	...
	public void removeForumSubscriptions(int forumId)
	{
		...
		String sql = "DELETE FROM jforum_topics_watch WHERE forum_id = ?";
		...
		{
			throw new RuntimeException("removeUserForumSubscriptions(forumId) - db write failed");
		}
	}
	...
	{
		String sql = "DELETE FROM jforum_topics_watch WHERE topic_id = ? AND forum_id = 0";
	...
	public void removeUserForumSubscriptions(int userId, int forumId)
	{
		...
		{
			throw new IllegalArgumentException("user id or forum id is missing");
		}
		
		String sql = "DELETE FROM jforum_topics_watch WHERE user_id = ? AND forum_id = ?";
		...
		if (!sqlService.dbWrite(sql, fields)) 
		{
			throw new RuntimeException("removeUserForumSubscriptions(userId, forumId) - db write failed");
		}
	}
	...
	public boolean removeUserPostLike(int postId, int userId)
	{
		...
		return postDao.deleteUserPostLike(postId, userId);
	}
	...
	{
		String sql = "DELETE FROM jforum_topics_watch WHERE topic_id = ? AND user_id = ? AND forum_id = 0";
	...
	public List<Topic> selectForumExportAndGradableTopics(int forumId)
	{
		...
		sql.append("FROM jforum_topics t, jforum_users u, jforum_posts p, jforum_posts p2, jforum_users u2 ");
		sql.append("WHERE t.forum_id = ? AND (topic_export = ? OR topic_grade = ?)");
		sql.append("AND t.user_id = u.user_id ");
		sql.append("AND p.post_id = t.topic_first_post_id ");
		sql.append("AND p2.post_id = t.topic_last_post_id ");
		sql.append("AND u2.user_id = p2.user_id ");
		...
		fields[i++] = Topic.TopicGradableCode.YES.getTopicGradableCode();
	...
	public int selectForumExportAndGradableTopicsCount(int forumId)
	{
		...
		sql.append("SELECT COUNT(1)AS topics_count ");
		sql.append("FROM jforum_topics t WHERE t.forum_id = ? AND (topic_export = ? OR topic_grade = ?)");
		...
		this.sqlService.dbRead(sql.toString(), fields, new SqlReader()
		{
			public Object readSqlResultRecord(ResultSet result)
			{
				...
				catch (SQLException e)
				{
					if (logger.isWarnEnabled())
					{
						logger.warn("selectForumExportAndGradableTopicsCount: " + e, e);
					}
					return null;
				}
			}
	...
	public List<Topic> selectForumGradableTopics(int forumId)
	{
		...
		sql.append("u2.user_id AS last_post_by_id, u2.user_fname AS last_post_by_fname, u2.user_lname AS last_post_by_lname, u2.user_avatar AS last_post_by_avatar, p2.post_time, p.attach ");
		sql.append("FROM jforum_topics t, jforum_users u, jforum_posts p, jforum_posts p2, jforum_users u2 ");
		sql.append("WHERE t.forum_id = ? AND topic_grade = ? ");
		sql.append("AND t.user_id = u.user_id ");
		sql.append("AND p.post_id = t.topic_first_post_id ");
		sql.append("AND p2.post_id = t.topic_last_post_id ");
		sql.append("AND u2.user_id = p2.user_id ");
	...
	{
		return getForumTopics(forumId, topicSort, startFrom, count);
	}
	...
	{
		List<Post> posts = getPostsByUser(siteId, userId);
	...
	public Set<Integer> selectUserCategoryPostLikes(int categoryId, int userId)
	{
		...
		sql.append("SELECT p.topic_id, pl.post_id, pl.user_id ");
		sql.append("FROM jforum_posts p, jforum_post_user_likes pl, jforum_forums f ");
		sql.append("WHERE f.categories_id = ? AND p.forum_id = f.forum_id AND p.post_id = pl.post_id AND pl.user_id = ?");
		...
		final Set<Integer> postIds = new HashSet<Integer>();
		...
		this.sqlService.dbRead(sql.toString(), fields, new SqlReader()
		{
			public Object readSqlResultRecord(ResultSet result)
			{
				...
				catch (SQLException e)
				{
					if (logger.isWarnEnabled())
					{
						logger.warn("selectUserCategoryPostLikes: " + e, e);
					}
					return null;
				}
			}
	...
	public Set<Integer> selectUserForumPostLikes(int forumId, int userId)
	{
		...
		sql.append("SELECT p.topic_id, pl.post_id, pl.user_id FROM jforum_posts p, jforum_post_user_likes pl WHERE p.forum_id = ? AND p.post_id = pl.post_id AND pl.user_id = ?");
		...
		this.sqlService.dbRead(sql.toString(), fields, new SqlReader()
		{
			public Object readSqlResultRecord(ResultSet result)
			{
				...
				catch (SQLException e)
				{
					if (logger.isWarnEnabled())
					{
						logger.warn("selectUserForumPostLikes: " + e, e);
					}
					return null;
				}
			}
	...
	public Set<Integer> selectUserTopicPostLikes(int topicId, int userId)
	{
		...
		sql.append("SELECT p.topic_id, pl.post_id, pl.user_id FROM jforum_posts p, jforum_post_user_likes pl WHERE p.topic_id = ? AND p.post_id = pl.post_id AND pl.user_id = ?");
		...
		this.sqlService.dbRead(sql.toString(), fields, new SqlReader()
		{
			public Object readSqlResultRecord(ResultSet result)
			{
				...
				catch (SQLException e)
				{
					if (logger.isWarnEnabled())
					{
						logger.warn("selectUserTopicPostLikes: " + e, e);
					}
					return null;
				}
			}
		...
		this.sqlService.transact(new Runnable()
		{
			public void run()
			{
				try
				{
					...
					if (topic != null)
					{
						// if user is not subscribed to forum then subscribe to topic
						if (!forumDao.isUserSubscribed(topic.getForumId(), userId))
						{
							// if user is not already subscribed then subscribe to topic
							if (!isUserSubscribed(topicId, userId))
							{
										
								String sql = "INSERT INTO jforum_topics_watch(topic_id, user_id, is_read, forum_id) VALUES (?, ?, '1', 0)";
	 ...
	 * {@inheritDoc}
	 */
	public void subscribeUser(final int topicId, final int userId, final int forumId)
	{
		if (topicId <= 0 || userId <= 0 || forumId <= 0)
		{
			throw new IllegalArgumentException("topic id or user id or forum id is missing");
		...
		this.sqlService.transact(new Runnable()
		{
			public void run()
			{
				try
				{
					// user can subscribe to forum topic subscription only if subscribed to the forum
					if (forumDao.isUserSubscribed(forumId, userId))
					{
					
						if (!isUserSubscribed(topicId, userId, forumId))
						{									
							String sql = "INSERT INTO jforum_topics_watch(topic_id, user_id, forum_id, is_read) VALUES (?, ?, ?, '1')";
							...
							if (!sqlService.dbWrite(sql, fields)) 
							{
								throw new RuntimeException("subscribeUser(topicId, userId, forumId) - db write failed");
							}
						}
					}
				}
		...
		}, "subscribeUser: topic id: " + topicId +" user id: "+ userId +" forum id: "+ forumId);
	}
	...
	{
		String sql = "UPDATE jforum_topics SET topic_export = ? WHERE topic_id = ?";
		
		Object[] fields = new Object[2];
		int i = 0;	
		
		fields[i++] = exportImportCode.getTopicExportImportCode();
		fields[i++] = topicId;		
			
		if (!sqlService.dbWrite(sql.toString(), fields)) 
		{
			throw new RuntimeException("updateExportImportCode: db write failed");
		}
	}
	...
	public void updateSubscribedTopicReadStatus(int topicId, int userId, boolean read)
	{
		...
		sql.append("UPDATE jforum_topics_watch SET is_read = ? WHERE topic_id = ? AND user_id = ?");
		...
		if (!sqlService.dbWrite(null, sql.toString(), fields))
		{
			throw new RuntimeException("updateSubscribedTopicReadStatus: db write failed");
		}
		...
		this.sqlService.transact(new Runnable()
		{
			public void run()
			{
				try
				{
					...
					// add topic subscriptions for forum subscription users
					try
					{
						if (forum != null)
						{
							List<Integer> forumSubscriptionUserIds = forumDao.selectForumSubscriptions(forum.getId());
							
							if (forumSubscriptionUserIds != null && !forumSubscriptionUserIds.isEmpty())
							{
								for (int userId : forumSubscriptionUserIds)
								{
									subscribeUser(topicId, userId, forum.getId());
								}
							}
						}
					}
	...
	abstract protected List<Topic> getForumTopics(int forumId, Topic.TopicSort topicSort, int startFrom, int count);
	...
	abstract protected List<Post> getPostsByUser(String siteId, int userId);
	...
	protected Post makePost(ResultSet rs) throws SQLException
	{
		...
		post.setSubject(JForumUtil.removeScriptTags(rs.getString("post_subject")));
		post.setText(JForumUtil.escapeScriptTag(JForumUtil.stripBodyTags(HtmlHelper.clean(this.getPostTextFromResultSet(rs), true))));
		...
		// likes count
		int userLikesCount = rs.getInt("user_likes_count");
		if (userLikesCount > 0)
		{
			post.setPostLikesCount(userLikesCount);
		}
		...
		this.sqlService.transact(new Runnable()
		{
			public void run()
			{
				try
				{
					...
					// remove all user topic subscriptions to watch as topics subscription may be part of forum subscription to watch forum topics
					if (fromForumId != toForumId)
					{
						deleteTopicSubscriptions(topic.getId());
					}
				}
	...
	/**
	 * Deletes the user forum subscription to the topic
	 * 
	 * @param topicId	Topic id
	 * 
	 * @param userId	User id
	 ...
	 * @param forumId	Forum id
	 */
	protected void removeUserForumSubscription(int topicId, int userId, int forumId)
	{
		if (topicId <= 0 || userId <= 0 || forumId <= 0)
		{
			throw new IllegalArgumentException("topic id or user id or forum id is missing");
		...
		String sql = "DELETE FROM jforum_topics_watch WHERE topic_id = ? AND user_id = ? AND forum_id = ?";
		...
		if (!sqlService.dbWrite(sql, fields)) 
		{
			throw new RuntimeException("removeUserForumSubscription(topicId, userId, forumId) - db write failed");
		}
	}
