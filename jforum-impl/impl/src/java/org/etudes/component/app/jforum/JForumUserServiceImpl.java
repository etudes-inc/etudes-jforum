/**********************************************************************************
 * $URL: https://source.etudes.org/svn/apps/jforum/tags/2.27/jforum-impl/impl/src/java/org/etudes/component/app/jforum/JForumUserServiceImpl.java $ 
 * $Id: JForumUserServiceImpl.java 12733 2016-07-21 21:53:34Z murthyt $ 
 *********************************************************************************** 
 * 
...
public class JForumUserServiceImpl implements JForumUserService
{
	...
	public List<User> getSiteUsers(String siteId)
	{
		...
		List<User> siteUsers = (List<User>)this.threadLocalManager.get("getSiteUsers:"+ siteId);
		
		if (siteUsers != null)
		{
			// make copy and return users
			List<User> siteUsersCopy = new ArrayList<User>();
			
			for (User user : siteUsers)
			{
				siteUsersCopy.add(new UserImpl((UserImpl)user));
			}
			
			return siteUsersCopy;
		}
		
		// sync with sakai site users list
		siteUsers = syncSakaiUsers(siteId, 0, 0);
		sortUsers(siteUsers);
		
		this.threadLocalManager.set("getSiteUsers:"+ siteId, siteUsers);
	...
	public List<User> getSiteUsers(String siteId, int startFrom, int count)
	{
		...
		List<User> siteUsers = (List<User>)this.threadLocalManager.get("getSiteUsers"+ startFrom +""+ count);
		...
		// sync with sakai site users list
		siteUsers = syncSakaiUsers(siteId, startFrom, count);
		this.threadLocalManager.set("getSiteUsers"+ startFrom +""+ count, siteUsers);
	 ...
	 * {@inheritDoc}
	 */
	public List<User> getUpdatedUserInfo (String siteId)
	{
		
		return updateUsersInfo(siteId, false);
	}
	
	/**
	 * {@inheritDoc}
	 */
	public int getUserCategoryPostsCount(int userId, int categoryId)
	{
		if (userId <= 0 || categoryId <= 0)
		{
			throw new IllegalArgumentException("user Id or category information is missing.");
		}
		
		String key = "jforum:userCategoryPostsCount:"+ String.valueOf(categoryId) +":"+ String.valueOf(userId);
		
		Integer userCategoryPostsCount =  (Integer) this.threadLocalManager.get(key);
		if (userCategoryPostsCount != null)
		{
			return userCategoryPostsCount.intValue();
		}
		
		userCategoryPostsCount = userDao.selectUserCategoryPostsCount(userId, categoryId);
		
		this.threadLocalManager.set(key, userCategoryPostsCount);
	 ...
	 * {@inheritDoc}
	 */
	public int getUserForumPostsCount(int userId, int forumId)
	{
		if (userId <= 0 || forumId <= 0)
		{
			throw new IllegalArgumentException("user Id or forum information is missing.");
		}
		
		String key = "jforum:getUserForumPostsCount:"+ String.valueOf(forumId) +":"+ String.valueOf(userId);
		
		Integer userForumPostsCount =  (Integer) this.threadLocalManager.get(key);
		if (userForumPostsCount != null)
		{
			return userForumPostsCount.intValue();
		}
		
		userForumPostsCount = userDao.selectUserForumPostsCount(userId, forumId);
		
		this.threadLocalManager.set(key, userForumPostsCount);
	...
	public int getUserSitePostsCount(int userId, String siteId)
	{
		...
		String key = "jforum:userSitePostsCount:"+ siteId +":"+ String.valueOf(userId);
	 ...
	 * {@inheritDoc}
	 */
	public int getUserTopicPostsCount(int userId, int topicId)
	{
		...
		String key = "jforum:getUserTopicPostsCount:"+ String.valueOf(topicId) +":"+ String.valueOf(userId);
		
		Integer userTopicPostsCount =  (Integer) this.threadLocalManager.get(key);
		if (userTopicPostsCount != null)
		{
			return userTopicPostsCount.intValue();
		}
		
		userTopicPostsCount = userDao.selectUserTopicPostsCount(userId, topicId);
		
		this.threadLocalManager.set(key, userTopicPostsCount);
	...
	public void modifyUser(User user)
	{
		...
		cleanedUserSignature = JForumUtil.escapeScriptTag(cleanedUserSignature);
	...
	{
		String key = "jforumuserservice:jforum:site:"+ siteId;
		
		Site site = (Site) this.threadLocalManager.get(key);
		...
		try
		{
			...
			this.threadLocalManager.set(key, site);
		}
		catch (IdUnusedException e)
		{
			if (logger.isWarnEnabled())
			{
				logger.warn("getSite: missing site: " + siteId);
			}
		}
	 ...
	 */
	protected List<User> updateUsersInfo (String siteId, boolean sortUsers)
	{
		Site site = getSite(siteId);
