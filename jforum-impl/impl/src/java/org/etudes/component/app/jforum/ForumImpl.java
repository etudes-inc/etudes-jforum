/**********************************************************************************
 * $URL: https://source.etudes.org/svn/apps/jforum/tags/2.27/jforum-impl/impl/src/java/org/etudes/component/app/jforum/ForumImpl.java $ 
 * $Id: ForumImpl.java 8257 2014-06-13 21:46:12Z murthyt $ 
 *********************************************************************************** 
 * 
...
{
	private static Log logger = LogFactory.getLog(ForumImpl.class);
	
	protected AccessDates accessDates = null;
	
	protected int accessType;
	
	protected Boolean allowTopicPostLikes = Boolean.FALSE;
	...
	protected boolean groupReadonlyUser = false;
	
	protected List<String> groups = null;
	
	protected int groupsReadonyAccessType;
	
	protected boolean groupUser = false;
	...
	public ForumImpl(JForumPostService jforumPostService, JForumCategoryService jforumCategoryService, JForumSecurityService jforumSecurityService, JForumGradeService jforumGradeService) 
	{
	...
	protected ForumImpl(ForumImpl other)
	{
		...
		if (other.groups != null)
		{
			...
			this.groupsReadonyAccessType = other.groupsReadonyAccessType;
			this.groupUser = other.groupUser;
			this.groupReadonlyUser = other.groupReadonlyUser;
		}
		...
		this.allowTopicPostLikes = other.allowTopicPostLikes;
		...
		this.topicOrder = other.topicOrder;
		...
		this.allowTopicPostLikes = other.allowTopicPostLikes.booleanValue();
		...
		this.jforumSecurityService = other.jforumSecurityService;
	...
	{
		if ((getGradeType() != Grade.GradeType.FORUM.getType()) || (getGrade() == null))
		{
			return null;
		}
		...
		try
		{
			...
			jforumGradeService.getForumEvaluationsWithPosts(this, Evaluation.EvaluationsSort.total_posts_a, currentSakaiUserId);
		}
		catch (JForumAccessException e)
		{
		...
		catch (Exception e)
		{
			...
			{
				logger.warn(this.getClass().getName()+".getEvaluationStatus() : Error while fetching forum evaluations: "+ e.toString());
				return null;
			}
		}
	...
	public Grade getGrade()
	{
		...
		if (getGradeType() == Grade.GradeType.FORUM.getType() && this.jforumGradeService != null && getId() > 0)
		{
			Grade grade = this.jforumGradeService.getByForumId(getId());
			this.setGrade(grade);
	 ...
	 * {@inheritDoc}
	 */
	public int getGroupsReadonyAccessType()
	{
	...
	public ForumDisplayStatus getUserDiplayStatus()
	{
		...
		* with regard to export, invisible forums are exported, even if they are not open yet, 
		...
		-if category due date or allow until date is in the past the forum is locked
		*/
		...
		if (currentSakaiUserId != null)
		{
			...
			if (category != null)
			{
				...
				if ((category.getAccessDates() != null && (category.getAccessDates().getAllowUntilDate() != null || category.getAccessDates().getDueDate() != null)))
				{
					if ((category.getAccessDates().getAllowUntilDate() != null) && (category.getAccessDates().getAllowUntilDate().before(currentTime)))
					{
						catClosed = true;
					}
		...
		else
		{
			...
			if (accessTypeRules)
			{
				if (catClosed)
				{
					if (this.accessType == ForumAccess.GROUPS.getAccessType())
					{
				...
				else if (this.accessType == ForumAccess.SITE.getAccessType())
				{
					if (this.type == ForumType.NORMAL.getType())
					{
						userStatusDisplay = (unread ? ForumDisplayStatus.FOLDER_NEW : ForumDisplayStatus.FOLDER);
	...
	{
		return allowTopicPostLikes;
	}

	/**
	 * {@inheritDoc}
	 */
	public boolean isGroupReadonlyUser()
	{
		return groupReadonlyUser;
	}
	...
	{
		this.allowTopicPostLikes = allowTopicPostLikes;
	}
	...
	/**
	 * @param groupReadonlyUser the groupReadonlyUser to set
	 */
	public void setGroupReadonlyUser(boolean groupReadonlyUser)
	{
		this.groupReadonlyUser = groupReadonlyUser;
	}
	 ...
	 * {@inheritDoc}
	 */
	public void setGroupsReadonyAccessType(int groupsReadonyAccessType)
	{
	...
	/**
	 * Sets the user evaluation for a gradable forum. If user posts are not graded the evaluation id is not set.
	 * 
