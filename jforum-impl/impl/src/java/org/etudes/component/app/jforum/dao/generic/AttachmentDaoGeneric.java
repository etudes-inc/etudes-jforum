/**********************************************************************************
 * $URL: https://source.etudes.org/svn/apps/jforum/tags/2.27/jforum-impl/impl/src/java/org/etudes/component/app/jforum/dao/generic/AttachmentDaoGeneric.java $ 
 * $Id: AttachmentDaoGeneric.java 5739 2013-08-27 22:31:41Z murthyt $ 
 *********************************************************************************** 
 * 
...
public abstract class AttachmentDaoGeneric implements AttachmentDao
{
	...
	{
		deletePrivateMessageAttachmentTx(privateMessageId, attachmentId);
	}
	 ...
	 * @param attachmentId	attachment id
	 */
	protected void deletePrivateMessageAttachmentTx(final int privateMessageId, final int attachmentId)
	{
		this.sqlService.transact(new Runnable()
		{
			public void run()
			{
				...
				if (!sqlService.dbWrite(sql, fields)) 
				{
					throw new RuntimeException("deletePrivateMessageAttachmentTx:deleteAttachmentDescription: db write failed");
				}
				...
				if (!sqlService.dbWrite(sql, fields)) 
				{
					throw new RuntimeException("deletePrivateMessageAttachmentTx:deleteAttachment: db write failed");
				}
				
				// draft attachment is associated with only one private message draft. When draft attachment is deleted the record in jforum_privmsgs_attach is to be deleted
				sql = "DELETE FROM jforum_privmsgs_attach WHERE attach_id = ?";
				fields = new Object[1];
				i = 0;
				fields[i++] = attachmentId;
				...
				if (!sqlService.dbWrite(sql, fields)) 
				{
					throw new RuntimeException("deletePrivateMessageAttachmentTx:deletePrivatemessageAttach : db write failed");
				}
				
				//update private message draft attach count
				sql = "SELECT COUNT(1) FROM jforum_privmsgs_attach WHERE privmsgs_id = ?";
				final List<Integer> privateMessageDraftAttachCountList = new ArrayList<Integer>();
				fields = new Object[1];
				i = 0;
				fields[i++] = privateMessageId;
				...
				sqlService.dbRead(sql.toString(), fields, new SqlReader()
				{
					public Object readSqlResultRecord(ResultSet result)
					{
						try
						{
							privateMessageDraftAttachCountList.add(result.getInt(1));
							return null;
						}
						catch (SQLException e)
						{
							if (logger.isWarnEnabled())
							{
								logger.warn("deletePrivateMessageAttachmentTx:selectPrivatemessageAttachCount: " + e, e);
							}
							return null;
						}
					}
				...
				int privateMessageDraftAttachAttachCount = 0;
				
				if (privateMessageDraftAttachCountList.size() == 1)
				{
					privateMessageDraftAttachAttachCount = privateMessageDraftAttachCountList.get(0);
				}
				
				sql = "UPDATE jforum_privmsgs SET privmsgs_attachments = ? WHERE privmsgs_id = ?";
				fields = new Object[2];
				i = 0;
				fields[i++] = privateMessageDraftAttachAttachCount;
				fields[i++] = privateMessageId;
				
				if (!sqlService.dbWrite(sql, fields)) 
				{
					throw new RuntimeException("deletePrivateMessageAttachmentTx:updatePrivateMessageDraftAttachmentCount: db write failed");
				}
