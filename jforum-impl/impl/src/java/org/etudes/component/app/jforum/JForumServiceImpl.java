/**********************************************************************************
 * $URL: https://source.etudes.org/svn/apps/jforum/tags/2.27/jforum-impl/impl/src/java/org/etudes/component/app/jforum/JForumServiceImpl.java $ 
 * $Id: JForumServiceImpl.java 12130 2015-11-25 15:23:12Z mallikamt $ 
 *********************************************************************************** 
 * 
...
public class JForumServiceImpl implements JForumService
{
	 ...
	 * {@inheritDoc}
	 */
	public Category getEvaluationsByCategory(String context, int categoryId, boolean includeDroppedBlocked)
	{
		if (context == null) throw new IllegalArgumentException();
		
		/*get category evaluations*/
		return this.jforumCategoryService.getCategoryEvaluations(context, categoryId, true);
	}
	...
	public Category getEvaluationsByForum(String context, int forumId, boolean includeDroppedBlocked)
	{
		...
		/*get forum evaluations*/
		return this.jforumCategoryService.getForumEvaluations(context, forumId, includeDroppedBlocked);
	}
	...
	{
		/*get topic evaluations*/
		return this.jforumCategoryService.getTopicEvaluations(context, topicId, includeBlockedDropped);
	}
	...
	public List<Category> getUserAccessibleGradableItemsByContext(String context, String userId)
	{
		...
		getUserContextGradableItems(context, userId, categories, true);
	...
	public List<Category> getCMUserFilteredGradableItemsByContext(String context, String userId)
	{
		...
		getUserGradableItems(context, userId, categories, true, true);
	...
	public List<Category> getCMUserGradableItemsByContext(String context, String userId)
	{
		...
		getUserGradableItems(context, userId, categories, false, true);
	...
	{
		return checkSpecialAccessOpenDate(specialAccess, false);
	}
	 ...
	 * 
	 * @param specialAccess	Special access
	 * @param callFromCM true means being invoked from student view of CM
	 * 
	 * @return	true - If call is not from CM student, if there is no open date or open date is before the current time or open date is 
	 *          after current time and not hidden
	 *          true - If call is from CM student, if there is no open date or open date is before/after current time
	 * 			false - if not accessible or open date is after the current time
	 */
	protected boolean checkSpecialAccessOpenDate(SpecialAccess specialAccess, boolean callFromCM)
	{
		...
		if (!callFromCM)
		{
			if ((specialAccess.getAccessDates().getOpenDate() == null)
					|| (specialAccess.getAccessDates().getOpenDate().before(currentTime) || (specialAccess.getAccessDates().getOpenDate()
							.after(currentTime) && !specialAccess.getAccessDates().isHideUntilOpen())))
			{
			return true;
		}
		}
	...
	/**
	 * Filters the items and adds user evaluation
	 * 
	 ...
	 * 
	 * @param validateDates	True Check dates else not
	 */
	protected void getUserContextGradableItems(String context, String userId, List<Category> categories, boolean validateDates)
	{
		...
		for (Iterator<Category> catIter = categories.iterator(); catIter.hasNext();) 
		{
			...
			if (category.isGradable() && (user != null))
			{
				...
				{				
					//TODO get user evaluation with first post time , last post and posts count
					((CategoryImpl)category).setUserEvaluation(this.jforumGradeService.getUserCategoryEvaluation(category.getId(), user.getSakaiUserId(), true));
			...
			else 
			{
				...
				for (Iterator<Forum> forumIter = forums.iterator(); forumIter.hasNext();) 
				{
				    ...
				    if (forum.getGradeType() == Grade.GradeType.FORUM.getType())
					{
						...
						{
							//TODO get user evaluation with first post time , last post and posts count
							((ForumImpl)forum).setUserEvaluation(this.jforumGradeService.getUserForumEvaluation(forum.getId(), user.getSakaiUserId(), true));
						}
					}
					else
					{
							...
							if (topic.isGradeTopic())
							{
								...
								*/
								...
								if (user !=null)
								{
									...
									{							
										//TODO get user evaluation with first post time , last post and posts count
										((TopicImpl)topic).setUserEvaluation(this.jforumGradeService.getUserTopicEvaluation(topic.getId(), user.getSakaiUserId(), true));
									}
								}
							}
						}
	...
	{
		getUserGradableItems(context, userId, categories, validateDates, false);
	}
	 ...
	 * @param	validateDates 	true	-	validate dates
	 * 							false	- ignore dates validation
	 * @param callFromCM true if being invoked from student CM view
	 *  
	 * @param categories
	 */
	protected void getUserGradableItems(String context, String userId, List<Category> categories, boolean validateDates, boolean callFromCM)
	{
		User user = getJforumUser(userId);
		...
		for (Iterator<Category> catIter = categories.iterator(); catIter.hasNext();) 
		{
			...
			{
				//if ((validateDates) && (category.getAccessDates() != null) && (category.getAccessDates().getOpenDate() != null) && (category.getAccessDates().getOpenDate().after(currentTime)))
				if ((validateDates) && !callFromCM && (category.getAccessDates() != null) && (category.getAccessDates().getOpenDate() != null) && (category.getAccessDates().getOpenDate().after(currentTime) && category.getAccessDates().isHideUntilOpen()))
				{
					catIter.remove();
					continue;
				}
						...
						if (forum.getAccessDates().getOpenDate() != null)
						{
							...
							if (forum.getAccessDates().getOpenDate().after(currentTime))
							{
								...
								if (user != null)
								{
									...
									if (specialAccessList != null)
									{
										for (SpecialAccess specialAccess : specialAccessList)
										{
											if (specialAccess.getUserIds().contains(new Integer(user.getId())))
											{
												...
												if (checkSpecialAccessOpenDate(specialAccess, callFromCM))
												{
													forum.getSpecialAccess().add(specialAccess);
													specialAccessUserAccess = true;
												}
									...
									{
										if (!callFromCM && forum.getAccessDates().isHideUntilOpen())
										{
											forumIter.remove();
											continue;
										}
				...
				{				
					...
					if (userSiteEvaluationsMap != null)
					{
						...
						if ((evaluation != null) && (evaluation.getUserId() == user.getId()))
						{
							...
							((CategoryImpl)category).setUserEvaluation(evaluation);
						}
					}
				}
			}
			else 
			{
				...
				for (Iterator<Forum> forumIter = forums.iterator(); forumIter.hasNext();) 
				{
				    ...
				    if (forum.getGradeType() == Grade.GradeType.FORUM.getType())
					{
						...
						// check category open date
						//else if ((validateDates) && (category.getAccessDates() != null) && (category.getAccessDates().getOpenDate() != null) && (category.getAccessDates().getOpenDate().after(currentTime)))
						else if ((validateDates) && !callFromCM && (category.getAccessDates() != null) && (category.getAccessDates().getOpenDate() != null) && (category.getAccessDates().getOpenDate().after(currentTime) && category.getAccessDates().isHideUntilOpen()))
						{
							forumIter.remove();
							continue;
						}
							...
							{							
								...
								if (userSiteEvaluationsMap != null)
								{
									...
									if ((evaluation != null) && (evaluation.getUserId() == user.getId()))
									{
										...
										((ForumImpl)forum).setUserEvaluation(evaluation);
									}
								}	
							}
						}
									...
									if (topic.getAccessDates().getOpenDate() != null)
									{
										...
										if (topic.getAccessDates().getOpenDate().after(currentTime))
										{
											...
											if (user != null)
											{
												...
												if (!specialAccessUser)
												{
													if (!callFromCM && topic.getAccessDates().isHideUntilOpen())
													{
														topicIter.remove();
														continue;
													}
									...
									{							
										...
										if (userSiteEvaluationsMap != null)
										{
											...
											if ((evaluation != null) && (evaluation.getUserId() == user.getId()))
											{
												...
												((TopicImpl)topic).setUserEvaluation(evaluation);
											}
										}
									}
		...
		this.sqlService.dbRead(sql.toString(), fields, new SqlReader()
		{
			public Object readSqlResultRecord(ResultSet result)
			{
				try
				{
				    ...
				    {
						Timestamp dueDate = result.getTimestamp("reviewed_date");
						((EvaluationImpl)evaluation).setReviewedDate(dueDate);
				    }
				    else
				    {
