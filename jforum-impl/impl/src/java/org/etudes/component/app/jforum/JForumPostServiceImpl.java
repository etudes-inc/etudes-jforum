/**********************************************************************************
 * $URL: https://source.etudes.org/svn/apps/jforum/tags/2.27/jforum-impl/impl/src/java/org/etudes/component/app/jforum/JForumPostServiceImpl.java $ 
 * $Id: JForumPostServiceImpl.java 12733 2016-07-21 21:53:34Z murthyt $ 
 *********************************************************************************** 
 * 
...
public class JForumPostServiceImpl implements JForumPostService
{
	...
	{
		if ((postId <= 0) || (sakaiUserId == null || sakaiUserId.trim().length() == 0))
		{
		...
		if (post == null)
		{
			throw new IllegalArgumentException("Post is not existing.");
		}
		...
		if (!forum.isAllowTopicPostLikes())
		{
			if (logger.isWarnEnabled())
			{
				logger.warn("Forum with id : "+ forum.getId() +" is not enabled to allow post likes.");
			}
			return false;
		}
		...
		if (!facilitator)
		{
			participant = jforumSecurityService.isJForumParticipant(category.getContext(), user.getSakaiUserId());
		}
		...
		return topicDao.addUserPostLike(postId, user.getId());
	}
	...
	public void assignZeroTopicNonSubmitters(int topicId, String evaluatedBySakaiUserId) throws JForumAccessException
	{
		...
		if (topic == null || !(topic.isGradeTopic()) || evaluatedBySakaiUserId == null || evaluatedBySakaiUserId.trim().length() == 0)
		{
			return;
		}
		...
		jforumGradeService.assignZeroTopicNonSubmitters(grade.getForumId(), grade.getTopicId(), evaluatedBySakaiUserId);
	...
	public int createTopic(Topic topic) throws JForumAccessException
	{
		...
		return createNewTopic(topic, true);
	}
	...
	public int createTopic(Topic topic, boolean notifications) throws JForumAccessException
	{
		...
		return createNewTopic(topic, notifications);		
	}
	...
	{
		if (forumId <= 0) throw new IllegalArgumentException("Forum id is not valid");
		
		return topicDao.selectForumExportAndGradableTopics(forumId);
	}
	...
	public int getForumExportAndGradableTopicsCount(int forumId)
	{
		...
		return topicDao.selectForumExportAndGradableTopicsCount(forumId);
	}
	...
	public List<Topic> getForumGradableTopics(int forumId)
	{
		...
		return topicDao.selectForumGradableTopics(forumId);
	}
	...
	public List<Topic> getRecentTopics(String context, int limit, String sakaiUserId) throws JForumAccessException
	{
		...
		boolean facilitator = jforumSecurityService.isJForumFacilitator(context, UserDirectoryService.getCurrentUser().getId());
		boolean isObserver = jforumSecurityService.isEtudesObserver(context, UserDirectoryService.getCurrentUser().getId());
		...
		// for participant user get evaluations for user for gradable items
		...
		if (user != null)
		{
			if (participant)
			{
				for (Topic topic : topics)
				{
					if (topic.isGradeTopic())
					{
						/* user topic evaluation */
						// get user site evaluations once
						if (userSiteEvaluationsMap == null)
						{
							userSiteEvaluationsMap = new HashMap<Integer, Evaluation>();
	
							userSiteEvaluations = jforumGradeService.getUserSiteEvaluations(topic.getForum().getCategory().getContext(), user.getId());
						...
						if (userSiteEvaluationsMap != null && topic.getGrade() != null)
						{
							Evaluation userEvaluation = userSiteEvaluationsMap.get(new Integer(topic.getGrade().getId()));
							
							// get topic user posts count
							int userTopicPostsCount = jforumUserService.getUserTopicPostsCount(user.getId(), topic.getId());
							
							if ((userEvaluation == null) || (userEvaluation.getUserId() != user.getId()))
							{
								userEvaluation = new EvaluationImpl();
								((EvaluationImpl)userEvaluation).setId(-1);
							}
							
							((EvaluationImpl)userEvaluation).setTotalPosts(userTopicPostsCount);								
							((TopicImpl)topic).setUserEvaluation(userEvaluation);
						}
					}
					else if (topic.getForum().getGradeType() == Grade.GradeType.FORUM.getType())
					{
						...
						if (userSiteEvaluationsMap != null && topic.getForum().getGrade() != null)
						{
							Evaluation userEvaluation = userSiteEvaluationsMap.get(new Integer(topic.getForum().getGrade().getId()));
							
							// get forum user posts count
							int userForumPostsCount = jforumUserService.getUserForumPostsCount(user.getId(), topic.getForum().getId());
							...
							((ForumImpl)topic.getForum()).setUserEvaluation(userEvaluation);
						}
					}
					else if (topic.getForum().getCategory().isGradable())
					{
						// user category evaluation
						// get user site evaluations once
						if (userSiteEvaluationsMap == null)
						{
							userSiteEvaluationsMap = new HashMap<Integer, Evaluation>();
						...
						if (userSiteEvaluationsMap != null && topic.getForum().getCategory().getGrade() != null)
						{
							Evaluation userEvaluation = userSiteEvaluationsMap.get(new Integer(topic.getForum().getCategory().getGrade().getId()));
							
							// get category user posts count
							int userCategoryPostsCount = jforumUserService.getUserCategoryPostsCount(user.getId(), topic.getForum().getCategory().getId());
							...
							((EvaluationImpl)userEvaluation).setTotalPosts(userCategoryPostsCount);								
							((CategoryImpl)topic.getForum().getCategory()).setUserEvaluation(userEvaluation);
						}
					}
				}
			...
			{
				// set current sakaiuser id to show the grading status
				for (Topic topic : topics)
				{
					if (topic.isGradeTopic())
					{
					...
					{
						((ForumImpl)topic.getForum()).setCurrentSakaiUserId(sakaiUserId);
					}
					else if (topic.getForum().getCategory().isGradable())
					{
						((CategoryImpl)topic.getForum().getCategory()).setCurrentSakaiUserId(sakaiUserId);
					}
				}
	...
	public Topic getTopic(int topicId, String sakaiUserId) throws JForumAccessException
	{
		...
		else 
		{
			...
			Evaluation userEvaluation = jforumGradeService.getUserTopicEvaluation(topicId, sakaiUserId);
	...
	public List<Post> getTopicPosts(int topicId, int startFrom, int count, String sakaiUserId) throws JForumAccessException
	{
		...
		Forum forum = jforumForumService.getForum(topic.getForumId(), sakaiUserId);
		if (forum == null)
		{
			return new ArrayList<Post>();
		}
		...
		Category category = jforumCategoryService.getCategory(forum.getCategoryId(), sakaiUserId);
		if (category == null)
		{
			return new ArrayList<Post>();
		}
		...
		boolean allowForumPostLikes = forum.isAllowTopicPostLikes();
		Set<Integer> userPostLikeIds = null;
		
		if (allowForumPostLikes)
		{
			// get posts like of current user
			User user = jforumUserService.getBySakaiUserId(sakaiUserId);
			userPostLikeIds = topicDao.selectUserTopicPostLikes(topicId, user.getId());
		}
		...
		for (Post post : posts)
		{
			...
			// post likes
			if (allowForumPostLikes && (userPostLikeIds != null))
			{
				if (userPostLikeIds.contains(post.getId()))
				{
					((PostImpl)post).setUserPostLike(Boolean.TRUE);
				}
			}
		}
		...
		if (!posts.isEmpty())
		{
			...
			// update subscribed user topic read status
			markSubscribeUserTopicStatusRead(topicId, sakaiUserId);
		}
		
		((TopicImpl)topic).setCurrentSakaiUserId(sakaiUserId);
		
		//get user evaluation
		if (topic.isGradeTopic() && participant)
		{
			User user = jforumUserService.getBySakaiUserId(sakaiUserId);
	...
	public List<Topic> getTopics(int forumId, int startFrom, int count, String sakaiUserId) throws JForumAccessException
	{
		...
		// user evaluations for participant users
		List<Evaluation> userSiteEvaluations = null;
		Map<Integer, Evaluation> userSiteEvaluationsMap = null;
		User user = jforumUserService.getBySakaiUserId(sakaiUserId);
		...
		for (Topic topic : topics)
		{
			...
			if (topic.isGradeTopic())
			{
		    	...
		    	// user evaluation for participant user
		    	if (participant && grade != null && user != null)
		    	{
		    		// get user site evaluations once
					if (userSiteEvaluationsMap == null)
					{
						...
						userSiteEvaluations = jforumGradeService.getUserSiteEvaluations(category.getContext(), user.getId());
	...
	public List<Topic> getTopics(int forumId, Topic.TopicSort topicSort, int startFrom, int count, String sakaiUserId) throws JForumAccessException
	{
		...
		List<Evaluation> userSiteEvaluations = null;
		Map<Integer, Evaluation> userSiteEvaluationsMap = null;
		List<Topic> topics = topicDao.selectForumTopics(forumId, topicSort, startFrom, count);
		User user = jforumUserService.getBySakaiUserId(sakaiUserId);
	...
	public List<Topic> getUserAccessibleTopics(int forumId, String sakaiUserId, TopicSort topicSort) throws JForumAccessException
	{
		...
		if (topicSort != null)
		{
			topics = topicDao.selectForumTopics(forumId, topicSort, 0, 0);
		}
		else
		{
			topics = topicDao.selectForumTopics(forumId);
		}
		...
		if (participant)
		{
			// filter user topics that are accessible
			filterUserAccessForumTopics(forum, sakaiUserId);
		}
	...
	{
		if (sakaiUserId == null || sakaiUserId.trim().length() == 0 || fetchedBySakaiUserId == null || fetchedBySakaiUserId.trim().length() == 0)
		{
			throw new IllegalArgumentException("Sakai user Information is missing or not valid.");
		}
		...
		// access check
		boolean fetchedByfacilitator = jforumSecurityService.isJForumFacilitator(category.getContext(), fetchedBySakaiUserId);
		...
		if ((!fetchedByfacilitator) && !(facilitator || participant))
		{
			throw new JForumAccessException(sakaiUserId);			
		}
		...
		boolean fetchedForumPostLikes = false;
		Set<Integer> userPostLikeIds = null;
		...
		for (Post post : posts)
		{
			...
			// post likes
			if (post.getTopic().getForum().isAllowTopicPostLikes())
			{
				if (!fetchedForumPostLikes)
				{
					// get posts like of current user only once
					User fetchedBySakaiUser = jforumUserService.getBySakaiUserId(fetchedBySakaiUserId);
					userPostLikeIds = topicDao.selectUserCategoryPostLikes(categoryId, fetchedBySakaiUser.getId());
				}
				
				if ((userPostLikeIds != null) && (userPostLikeIds.contains(post.getId())))
				{
					((PostImpl)post).setUserPostLike(Boolean.TRUE);
				}
			}
	...
	public int getUserCategoryPostsCount(int categoryId, String sakaiUserId) throws JForumAccessException
	{
		...
		// category user posts count
		int userCategoryPostsCount = jforumUserService.getUserCategoryPostsCount(user.getId(), category.getId());
		
		return userCategoryPostsCount;
	}
	...
	public List<Post> getUserForumPosts(int forumId, String sakaiUserId, String fetchedBySakaiUserId) throws JForumAccessException
	{
		if (sakaiUserId == null || sakaiUserId.trim().length() == 0 || fetchedBySakaiUserId == null || fetchedBySakaiUserId.trim().length() == 0)
		{
			throw new IllegalArgumentException("Sakai user information or fetched by user information is missing or not valid.");
		}
		...
		if (allowForumPostLikes)
		{
			// get posts like of current user
			User fetchedBySakaiUser = jforumUserService.getBySakaiUserId(fetchedBySakaiUserId);
			userPostLikeIds = topicDao.selectUserForumPostLikes(forumId, fetchedBySakaiUser.getId());
		}
	...
	public int getUserForumPostsCount(int forumId, String sakaiUserId) throws JForumAccessException
	{
		...
		// access check
		boolean facilitator = jforumSecurityService.isJForumFacilitator(forum.getCategory().getContext(), sakaiUserId);
		
		boolean participant = false;
		if (!facilitator)
		{
			participant = jforumSecurityService.isJForumParticipant(forum.getCategory().getContext(), sakaiUserId);
		}
	...
	public List<Post> getUserPosts(String siteId, String sakaiUserId) throws JForumAccessException
	{
		...
		{
			throw new IllegalArgumentException("Site Information is missing or not valid.");
		}
		...
		if (!facilitator)
		{
			participant = jforumSecurityService.isJForumParticipant(siteId, sakaiUserId);
		}
		...
		List<Post> posts = topicDao.selectPostsByUser(siteId, user.getId());
		
		// posted by user posts count
		int userSitePostsCount = jforumUserService.getUserSitePostsCount(user.getId(), siteId);
	...
	public List<Post>  getUserTopicPosts(int topicId, String sakaiUserId, String fetchedBySakaiUserId) throws JForumAccessException
	{
		...
		if (allowForumPostLikes)
		{
			// get posts like of current user
			User fetchedBySakaiUser = jforumUserService.getBySakaiUserId(fetchedBySakaiUserId);
			userPostLikeIds = topicDao.selectUserTopicPostLikes(topicId, fetchedBySakaiUser.getId());
		}
	...
	public int getUserTopicPostsCount(int topicId, String sakaiUserId) throws JForumAccessException
	{
		...
		// access check
		boolean facilitator = jforumSecurityService.isJForumFacilitator(topic.getForum().getCategory().getContext(), sakaiUserId);
		
		boolean participant = false;
		if (!facilitator)
		{
			participant = jforumSecurityService.isJForumParticipant(topic.getForum().getCategory().getContext(), sakaiUserId);
		}
	...
	public void modifyTopic(Topic topic, String bySakaiUserId) throws JForumItemNotFoundException, JForumAccessException, JForumGradesModificationException
	{
		...
		Topic exisTopic = getTopic(topic.getId());
		
		if (exisTopic == null)
		{
			throw new JForumItemNotFoundException(topic.getId());
		}
		
		if (topic.getForumId() != exisTopic.getForumId())
		{
			throw new IllegalArgumentException("Topic forum is not same as existing topic forum");
		}
		...
		boolean facilitator = jforumSecurityService.isJForumFacilitator(category.getContext(), bySakaiUserId);
		...
		if (exisTopic.isGradeTopic() && exisTopic.getGrade().isAddToGradeBook())
		{
			...
			topic.getGrade().setAddToGradeBook(exisTopic.getGrade().isAddToGradeBook());
		}
		else					
		{
			if (exisGrade != null)
	...
	public void modifyTopicPost(Post post) throws JForumAccessException, JForumAttachmentOverQuotaException, JForumAttachmentBadExtensionException, JForumGradesModificationException
	{
		...
		if (exisTopic.getFirstPostId() == post.getId())
		{
			...
			if ((exisTopic.isGradeTopic()) && (exisTopic.getGrade() != null && exisTopic.getGrade().isAddToGradeBook()) && (site != null)
									&& ((site.getToolForCommonId(JForumGradeService.GRADEBOOK_TOOL_ID) == null) && (etudesGradebookService.isToolAvailable(category.getContext()))))
			{
				...
				if (!etudesGradebookService.isTitleDefined(category.getContext(), post.getPostedBy().getSakaiUserId(), exisTopic.getTitle(), String.valueOf(topic.getId()), GradebookItemType.topic))
				{
					if (!etudesGradebookService.isTitleAvailable(category.getContext(), post.getPostedBy().getSakaiUserId(), exisTopic.getTitle()))
					{
						exisTopic.getGrade().setAddToGradeBook(Boolean.FALSE);
					}
				}
			}
	...
	public void modifyTopicReuse(int[] topicId, Topic.TopicExportImportCode exportImportCode)
	{
		...
		for (int count = 0; count < topicId.length; count++) 
		{
			topicDao.updateExportImportCode(topicId[count], exportImportCode);
		}
	}
	...
	public boolean removePostLike(int postId, String sakaiUserId) throws JForumAccessException
	{
		...
		return topicDao.removeUserPostLike(postId, user.getId());
	}
	 ...
	 * {@inheritDoc}
	 */
	protected int createNewTopic(Topic topic, boolean notifications) throws JForumAccessException
	{
		if (topic == null)
		{
			throw new IllegalArgumentException();			
		...
		if (topic.isGradeTopic())
		{
			if ((topic.getGrade() != null) && (topic.getGrade().getPoints() != null))
			{
				...
				// check duplicate titles in etudes gradebook
				if ((topic.getGrade().isAddToGradeBook()) && (site != null)
										&& ((site.getToolForCommonId(JForumGradeService.GRADEBOOK_TOOL_ID) == null) && (etudesGradebookService.isToolAvailable(category.getContext()))))
				{
					if (!etudesGradebookService.isTitleAvailable(category.getContext(), topic.getPostedBy().getSakaiUserId(), topic.getTitle()))
					{
						topic.getGrade().setAddToGradeBook(Boolean.FALSE);
					}
				}
			}
			else
		...
		// deny access forums sends no notifications
		if (forum.getAccessType() == Forum.ForumAccess.DENY.getAccessType())
		{
			notifyTopicToUsers = false;
		}
	...
	/**
	 * Removes the topics that are not accessible by the user
	 * 
	 ...
	 */
	protected void filterUserAccessForumTopics(Forum forum, String sakaiUserId) throws JForumAccessException
	{
		if (forum == null)
		{
			return;
		...
		Iterator<Topic> topicsIter = forum.getTopics().iterator();
	...
	/**
	 * Marks subscribed topic as unread
	 * 
	 * @param topicId	Topic id
	 * 
	 * @param sakaiUserId	Sakai user id
	 */
	protected void markSubscribeUserTopicStatusRead(int topicId, String sakaiUserId)
	{
		if ((topicId <= 0) || (sakaiUserId == null || sakaiUserId.trim().length() == 0))
		{
			throw new IllegalArgumentException("markSubscribeUserTopicStatusRead: topic or user information is missing.");
		}
		...
		topicDao.updateSubscribedTopicReadStatus(topicId, user.getId(), true);
	}
	...
	{
		List<User> usersToNotify = jforumUserService.getUpdatedUserInfo(category.getContext());
		...
		// forum watch users
		List<Integer> forumSubscribers = forumDao.selectForumSubscriptions(topic.getForumId());
		
		Map<Integer, Integer> forumSubscribersMap = new HashMap<Integer, Integer>();
		for (Integer forumSubscriber : forumSubscribers) 
		{
			forumSubscribersMap.put(forumSubscriber,forumSubscriber);		
		}
		...
		while (usersIterator.hasNext())
		{
			...
			if (sendToGroups && !jforumSecurityService.isJForumFacilitator(category.getContext(), usr.getSakaiUserId())) 
			{
				...
				for (Iterator<Group> usrGrpIter = sakaiSiteGroups.iterator(); usrGrpIter.hasNext();)
				{
					...
					{
						userInGroup = true;
						((ForumImpl)topicForum).setGroupUser(true);
						break;
					}
				}
			...
			// send notification if user's preference is to receive to new topic notifications or subscribed to topic's forum
			...
			{
				if (forumSubscribersMap.containsKey(usr.getId()))
				{
					if ((usr.getEmail() == null || usr.getEmail().trim().length() == 0) || (!jforumSecurityService.isUserActive(category.getContext(), usr.getSakaiUserId())))
					{
						usersIterator.remove();
		...
		if (usersToNotify != null && usersToNotify.size() > 0) 
		{
			...
			String subject = topic.getTitle() +" - New Post ("+ site.getTitle() +")";
	...
	{
		List<User> siteUsers = jforumUserService.getUpdatedUserInfo(category.getContext());
		...
		if (usersToNotify != null && usersToNotify.size() > 0) 
		{
			...
			{
				postFrom = post.getPostedBy().getFirstName() +" "+ post.getPostedBy().getLastName();		
			}
	...
	protected void updateGradeBook(Topic topic)
	{
		...
		try
		{
			...
			if ((site != null) && (site.getToolForCommonId(JForumGradeService.GRADEBOOK_TOOL_ID) == null))
			{
				return;
			}
		}
		...
		try
		{
			...
			*/
			...
			String gradebookToolId = JForumGradeService.GRADEBOOK_TOOL_ID;
